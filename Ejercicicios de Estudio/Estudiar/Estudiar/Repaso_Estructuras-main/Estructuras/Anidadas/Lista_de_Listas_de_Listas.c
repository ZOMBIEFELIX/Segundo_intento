...
struct lista_listas_listas
{
	int ref;
	struct lista_listas *sLiList;
	struct lista_listas_listas *next;
};

typedef struct lista List;
typedef struct lista_listas LiList;
typedef struct lista_listas_listas LiLiList;
...
//Funciones de la lista de listas de listas
int insertLL(LiLiList **startLL, int N);
int seekLL(LiLiList *startLL);
List *searchLL(LiLiList *startLL, int N);
int deleteLL(LiLiList **startLL, int N);


int main(int argc, char const *argv[])
{
	LiLiList *lililist = NULL;
	...
	do
	{
		...
		switch(option)
		{
			...
			case 3: 
				...
				List *item = searchLL(lililist, number);
				if (item)
					printf("Founded: %d \n\n", item->data);
				else
					printf("No Exist!!!\n\n");
...
void menu()
{
	printf("Select your option to choose from the list of lists of lists: \n\t");
	printf("[1]: Insert\n\t[2]: Show\n\t[3]: Search\n\t[4]: Delete\n\t[0]: Exit\n> ");
}
...
// Implementacion funciones lista de listas de listas
int insertLL(LiLiList **startLL, int N)
{
	if(!*startLL)
	{
		LiLiList *new = (LiLiList *) malloc(sizeof(LiLiList));
		if(!new)
			return 0;
		new->ref = N;
		new->next = *startLL;
	...
	if((*startLL)->ref > N)
	{
		if(!insertL(&((*startLL)->sLiList), N))
			return 0;
		return 1;
	}
	return insertLL(&(*startLL)->next, N);
}
...
int seekLL(LiLiList *startLL)
{
	if(!startLL)
		return 0;
	seekL(startLL->sLiList);
	printf(" \n | \n V \n");
	return seekLL(startLL->next);
...
List *searchLL(LiLiList *startLL, int N)
{
	if(!startLL)
		return 0;
	List *founded = searchL(startLL->sLiList, N);
	if(founded)
		return founded;
	return searchLL(startLL->next, N);
}
...
int deleteLL(LiLiList **startLL, int N)
{
	if(!*startLL)
		return 0;
	if(deleteL(&(*startLL)->sLiList, N) == 1)
	{
		if(!(*startLL)->sLiList)
		{
			LiLiList *aux = *startLL;
		...
		return deleteLL(&(*startLL)->next, N);
	return 0;
}